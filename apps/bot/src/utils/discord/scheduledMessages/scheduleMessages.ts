...
export function scheduleTomorrowEventMessage(options: scheduleTomorrowEventMessageOptions) {
	...
	scheduleJob(getEventTomorrowAnnoucementTime(), async () => {
		const sentMesssage = await options.annoucementChannel?.send({
			embeds: [
				getEventTomorrowEmbed({ event: options.event, avatar: options.avatarURL }),
			],
		});
		if (sentMesssage?.crosspostable) {
			sentMesssage.crosspost();
		}
	});
}
...
export function scheduleTodayEventMessage(options: scheduleTodayEventMessageOptions) {
	...
	scheduleJob(annouceTime, async () => {
		const sentMesssage = await options.annoucementChannel?.send(getEventTodayMessage({
			event: options.event,
